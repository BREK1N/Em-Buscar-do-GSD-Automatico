...
class RelsisamView(TemplateView):
    template_name = 'SCIM/relsisam.html'

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        hoje = date.today()

        # IDs de todos os efetivos em missão hoje
        missoes_hoje = Missao.objects.filter(data_missao=hoje)
        em_missao_ids = set()
        for m in missoes_hoje.prefetch_related('equipe'):
            em_missao_ids.update(m.equipe.values_list('id', flat=True))
            if m.cmt_missao_id:
                em_missao_ids.add(m.cmt_missao_id)
            if m.motorista_id:
                em_missao_ids.add(m.motorista_id)

        tipos = TipoCurso.objects.filter(ativo=True).order_by('nome')
        dados = []
        for tipo in tipos:
            efetivos_ids = set(
                CursoEfetivo.objects.filter(tipo_curso=tipo).values_list('efetivo_id', flat=True).distinct()
            )
            total = len(efetivos_ids)
            em_missao = len(efetivos_ids & em_missao_ids)
            disponiveis = total - em_missao

            # Nomes dos efetivos em missão hoje com esse curso
            em_missao_lista = Efetivo.objects.filter(
                id__in=efetivos_ids & em_missao_ids
            ).values('posto', 'nome_guerra') if em_missao else []
            ...
            dados.append({
                ...
                'em_missao_lista': list(em_missao_lista),
            })

        ctx['dados'] = dados
        ctx['hoje'] = hoje
        ctx['total_em_missao'] = len(em_missao_ids)
